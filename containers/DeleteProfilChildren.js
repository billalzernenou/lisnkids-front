...
export default function DeleteProfilChildren({
  dayNightMode,
  setSettingScreen,
      ...
      try {
        ...
        if (response.data) {
          if (response.data.childrens.length !== 0) {
            ...
            // redirection vers la page setting si success
            setParentMode(true);
  ...
  // Edit children
  const deleteChildren = async (firstNameChildren) => {
    setError(null);
    try {
      const formData = new FormData();
      // const userId = await AsyncStorage.getItem("userId");
  ...
  return isLoading ? (
    ...
    >
      ...
      <View>
        <MainTitle
          text={`Cliquez sur le profil enfant à supprimer`}
          color="white"
          fontSize={16}
          textAlign="center"
          marginBottom={8}
