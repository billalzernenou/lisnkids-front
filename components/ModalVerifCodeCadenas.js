...
const modalVerifSecretCode = ({
    ...
    const verifSecretCode = async () => {
        setError(null);
        if (textInput) {
            if (Number(data1) === Number(textInput)) {
                console.log(data1);
                if (data0 !== "PlayList vide" && data0.length !== 0) {
                    setValue(!value);
                    setSwitchProtected(true);
                    navigation.navigate("ProtectedNightMode", {
                        item: data0[0],
                        data: data0,
                    });
                } else {
                    alert("Vous n'avez pas d'épisode dans votre Playlist");
                }
            } else {
                setError("Code errone");
            }
        } else {
